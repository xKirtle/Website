  ...
  $(document).ready(function(){   
   $('button#spielen').hover(function(){
    $(this).stop().animate({'width': '154px', 'height': '44px', 'margin': '-22px 0 0 -77px', 'font-size': '1.705em'}, 100);
   }, function(){
    $(this).stop().animate({'width': '140px', 'height': '40px', 'margin': '-20px 0 0 -70px', 'font-size': '1.55em'}, 100);
   });  
   ...
   $('#gameovermenu button').hover(function(){
    ...
    $(this).stop().animate({'width': button.width * 1.05, 'height': button.height * 1.05, 'font-size': button.font_size * 1.05}, 100);
   }, function(){
   ...
   $('#gameovermenu button#teilen').click(function(){
    this.fenster = window.open('bothotel.in', 'bot', 'width=500, height=200');
    this.fenster.focus();
   });
  ...
  var spieler = {
   ...
   zeichnen: function(){
    switch(this.richtung){
	 case 'hinten':
      this.bild_daten = new Array(10, -25, spieler.x + 2, spieler.y - 65);
	 break;
	 
	 case 'vorne':
	  this.bild_daten = new Array(10, -25, spieler.x + 2, spieler.y - 75);
	 break;	 
	 
	 case 'links':
	  this.bild_daten = new Array(10, -25, spieler.x + 7, spieler.y - 67);
	 break;	 
	 
	 case 'rechts':
	  this.bild_daten = new Array(10, -25, spieler.x + 5, spieler.y - 67);
	 break;
	 
	 case 'hintenlinks':
	 case 'hintenrechts':
	  this.bild_daten = new Array(10, -25, spieler.x + 5, spieler.y - 65);
	 break;	 
	 
	 case 'vornelinks':
	  this.bild_daten = new Array(10, -15, spieler.x + 12, spieler.y - 65);
	 break;	 
	 
	 case 'vornerechts':
	  this.bild_daten = new Array(10, -15, spieler.x + 0, spieler.y - 65);
	 break;
	}
	...
	$('#blackscreen').animate({'opacity': '1'}, 250, function(){
	 ...
	 window.setTimeout(function(){
	  $('#blackscreen').animate({'opacity': '0'}, 250, function(){
	   ...
	   level.fortschritt.aktuell = 0;
	   spiel.lvl_beendet = 0;
       $('#spielmenu').append('<div id="lvl_up" style="left: -90px">Você passo para o nivel ' + (level.aktuell) + ' !</div>');
    ...
    switch(objekt){
	 ...
	 case 'steine':
	  ...
	  if(spieler.leben == 0){
       ...
       animation.animationen['explosionen'].push({'X': x, 'Y': y, 'maße': [[45, 30, 37, 39], [174, 19, 65, 62], [13, 133, 101, 84], [155, 133, 103, 83], [287, 135, 105, 80], [8, 245, 110, 98], [143, 235, 128, 117], [277, 249, 126, 92], [148, 374, 120, 72]], 'frame': 1, 'timestamp': Date.now(), 'intervals': [80, 80, 60, 60, 60, 60, 60, 50, 50]});
  ...
  var spiel = {
   ...
   bilder_laden: function(){
    spiel.bild_laden('spielbg1', './_images/minegame/spielbg1.png');
    spiel.bild_laden('spielbg2', './_images/minegame/spielbg2.png');
    spiel.bild_laden('spielbg3', './_images/minegame/spielbg3.png');
    spiel.bild_laden('spielbg4', './_images/minegame/spielbg4.png');
    spiel.bild_laden('auto-hinten', './_images/minegame/auto-hinten.png');
    spiel.bild_laden('auto-vorne', './_images/minegame/auto-vorne.png');
    spiel.bild_laden('auto-links', './_images/minegame/auto-links.png');
    spiel.bild_laden('auto-rechts', './_images/minegame/auto-rechts.png');
    spiel.bild_laden('auto-vornelinks', './_images/minegame/auto-vornelinks.png');
    spiel.bild_laden('auto-vornerechts', './_images/minegame/auto-vornerechts.png');    
	spiel.bild_laden('auto-hintenlinks', './_images/minegame/auto-hintenlinks.png');
    spiel.bild_laden('auto-hintenrechts', './_images/minegame/auto-hintenrechts.png');
	spiel.bild_laden('avatar-hinten', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=7&head_direction=7');
	spiel.bild_laden('avatar-hintenlinks', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=6&head_direction=6');
	spiel.bild_laden('avatar-links', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=5&head_direction=5');
	spiel.bild_laden('avatar-vornelinks', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=4&head_direction=4');
	spiel.bild_laden('avatar-vorne', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=3&head_direction=3');
	spiel.bild_laden('avatar-icon', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=3&head_direction=3&size=s');
	spiel.bild_laden('avatar-vornerechts', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=2&head_direction=2');
	spiel.bild_laden('avatar-rechts', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=1&head_direction=1');
	spiel.bild_laden('avatar-hintenrechts', 'http://habbo.nl/habbo-imaging/avatarimage?figure=' + spieler.aussehen + '&direction=0&head_direction=0');
    spiel.bild_laden('taler', './_images/minegame/taler.png');
    spiel.bild_laden('steine', './_images/minegame/stein.png');
	spiel.bild_laden('explosionen', './_images/minegame/explosion.png');
	spiel.bild_laden('herzen', './_images/minegame/herz.png');
   },
   
   sound_laden: function(name, schleife, volume){
    this.musik[name] = new Audio('./_images/minegame/sons/' + name + '.mp3');
   ...
   menu_wechseln: function(menu){
    ...
    switch(menu){
	 case 'spiel':
	  ...
	  setTimeout(function(){
	   $('img#pfeiltasten').fadeOut(500, function(){ $(this).remove(); });
	  }, 2500);
	 break;
    ...
    spiel.render = function(){
	 ...
	 // Level Anzeige
	  // Linie
	   ...
	   main_ctx.strokeStyle = '#71706a';
	   main_ctx.fillStyle = '#46453e';
	   main_ctx.fill();
	   main_ctx.closePath();
	   main_ctx.stroke();
      ...
      // Beschriftung: Box links (aktuelles Level)	   
       ...
       main_ctx.fillText('Nv. ' + level.aktuell, 12, 31);
       main_ctx.closePath();	   
       main_ctx.stroke();	   
	  ...
	  // Beschriftung: Box rechts (nächstes Level)	   
	   ...
	   main_ctx.font = '0.8em Ubuntu';
	   main_ctx.fillStyle = 'white';
       main_ctx.fillText('Nv. ' + (level.aktuell + 1), 202, 31);
	   main_ctx.closePath();
	   main_ctx.stroke();
	  ...
	  // Level Fortschritt Box
	   ...
	   main_ctx.strokeStyle = '#FFF';
	   main_ctx.fillStyle = '#d8d8d8';
  ...
  document.addEventListener("keydown", function(e) { spiel.tasten[e.keyCode] = true; }, false);
  document.addEventListener("keyup", function(e) { delete spiel.tasten[e.keyCode]; }, false);