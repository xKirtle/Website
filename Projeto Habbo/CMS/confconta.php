...
if($_POST['acao'] == 'confirma'){
$remetente = $_POST['email'];
$usuarior = $_POST['usuario'];
$codigo = $_POST['codigo'];
$senha = HoloText($_POST['novasenha']);
$buscar_usuario = mysql_query("SELECT * FROM users WHERE username='$usuarior' AND mail='$remetente' AND Lalacodcon='$codigo' AND password = MD5(SHA1(MD5('" . $senha . "')))");
...
$data = $relatorio['account_created'];
  ...
  <strong>Erro!</strong> Algo esta errado ".$usuarior."!
		...
		mysql_query("UPDATE users SET LalaConf = '1' WHERE username = '".$usuarior."'");

			
			echo ("<div class='container'><div class='alert alert-success fade in'>
 <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
<strong>Enviado!</strong> Sua conta foi confirmada com exito! ".$usuarior."!
</div></div>");
...
<div class="panel-heading">Confirma Conta</div>
...
<div class="form-group"> 
<label for="username"><b>Senha:</b></label> 
<input class="form-control" id="novasenha" name="novasenha" type="password" size="35" placeholder="Digite a senha do usuario"  required> 
</div>
<div class="form-group"> 
<label for="username"><b>Repetir a senha:</b></label> 
<input class="form-control" id="confirmacaonovasenha" name="confirmacaonovasenha" type="password" size="35" placeholder="Repetir a senha do usuario"  required> 
</div>
<div class="form-group">
<input type="submit" value="Confirma Conta" name="register" id="doregister" class="btn btn-success" >
<input type="hidden" id="confirma" size="35" name="confirma" value="sendToken"> 
</div>
</form> 
</div>
</div>
...
<div class="panel-heading">Tenha mais vantagem com conta confimada.</div>
<div class="panel-body">
