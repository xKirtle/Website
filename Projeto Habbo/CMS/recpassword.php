...
$buscar_usuario = mysql_query("SELECT * FROM users WHERE username='$usuarior' AND mail='$remetente'");
$verifica = mysql_num_rows($buscar_usuario);
while($relatorio = mysql_fetch_array($buscar_usuario)){
$id = $relatorio['id'];
$usuario = $relatorio['username'];
$codigo = $relatorio['password'];
$data = $relatorio['account_created'];
  ...
  <strong>Erro!</strong> Esse email não é o mesmo da conta ".$usuarior."!
</div></div>");

		}else{
		mysql_query("UPDATE users SET LalaRecu = '1' WHERE username = '".$usuarior."'");
			$assunto = 'Habbz - Recuperação de senha';
			...
			$mensagem .= '<b>Código: </b>'.$codigo.'<br>';
			$mensagem .= '<b>Data de nascimento: </b>'.$data.'<br><br>';
			$mensagem .= '<b>Para recuperar sua conta clique <a href="http://Habbz.biz/recuperando.php">aqui</a>, e coloque seu nick, e-mail e o codigo é sua nova senha.</b></br>';
			$mensagem .= '<br>';
			$mensagem .= 'Atenciosamente, Suporte/moderação - Habbz<br><br>';
			$mensagem .= 'Caso você não tenha solicitado a recuperação de senha, iguinore este email.<br>';
 ...
 <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
<strong>Enviado!</strong> Você acaba de envia um recuperador de senha para o seu email ".$remetente."!
</div></div>");
...
<div class="form-group">
<input type="submit" value="Enviar Token por email" name="register" id="doregister" class="btn btn-success" >
<input type="hidden" id="formulario" size="35" name="formulario" value="sendToken"> 
</div>
</form> 
</div>
</div>
